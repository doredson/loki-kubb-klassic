...
import net.oredson.loki.kubb.activities.EditPlayerActivity;
...
public class PlayerListFragment extends ListFragment implements LoaderCallbacks<Cursor> {
	boolean mDualPane;
	...
	void showDetails(int index) {
		...
		String rowId = null;
		if ( index > -1 ) {
			rowId = cursor.getString(cursor.getColumnIndexOrThrow(PlayerTable.COLUMN_ID));
		}
		...
		if (mDualPane) {
			...
			PlayerDetailsFragment details = (PlayerDetailsFragment)
					getFragmentManager().findFragmentById(R.id.details);
			if (details == null || details.getShownItem() == null || !details.getShownItem().equals(rowId)) {
				details = PlayerDetailsFragment.newInstance(rowId);
			...
			intent.setClass(getActivity(), EditPlayerActivity.class);
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		CursorLoader cursorLoader = new CursorLoader(getActivity(),
				KubbContentProvider.PLAYERS_CONTENT_URI, projection, null, null, null);
		return cursorLoader;
	}
